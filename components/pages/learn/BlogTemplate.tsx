...
const BlogTemplate = async ({ blog, heading, editHref }: BlogTemplateProps) => {
  ...
  return (
    <article className="learn-article-surface p-6 sm:p-8 md:p-10">
      <div className="mb-4 flex items-start justify-between gap-4">
        <h1
          id="blog-title"
          className="scroll-mt-28 font-[family-name:var(--font-display)] text-2xl font-extrabold tracking-tight text-slate-900 sm:text-3xl md:text-4xl"
        >
          {heading}
        </h1>
        {editHref ? (
          <Link
            href={editHref}
            className="shrink-0 rounded-lg border border-violet-200 bg-violet-50 px-3.5 py-1.5 text-xs font-bold text-violet-700 transition hover:border-violet-300 hover:bg-violet-100"
          >
            Edit
          </Link>
        ) : null}
